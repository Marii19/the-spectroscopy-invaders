import Defender from "../../game_objects/defender";
import Field from "../../game_objects/field";
import Hero from "../../game_objects/hero";
import level from "../level";

export default class possibleFutureFour extends level{

    constructor(){
        let instructions: string[] = ['possible-future', 'move conjunct info', "move negation info", 'move observation info']
        super('possible-future four', 'a.b.c.0+a.b.d.0', 'a.(b.c.0+b.d.0)', 'Possible-future', instructions);
        this.enemy = "enemy_possible_future"
    }

    preload(){
        super.preload();
        this.load.image('background_possible_future', 'assets/backgrounds/level_backgrounds/background_possible_future.png');
        this.load.spritesheet('enemy_possible_future', 'assets/characters/enemy_possible_future.png', { frameWidth: 32, frameHeight: 48 });
    }

    create(){
        this.add.image(400, 300, 'background_possible_future');
        super.create();
 
        // Fields
        let field_0: Field = this.createField(200, 200, 'a.b.c.0+a.b.d.0', 0);

        this.createField(100, 320, 'b.c.0', 1);

        this.createField(300, 320, 'b.d.0', 2);

        this.createField(100, 440, 'c.0', 3);

        this.createField(300, 440, 'd.0', 4);

        this.createField(200, 560, '0', 5);

        let field_defender: Field = this.createField(600, 200, 'a.(b.c.0+b.d.0)', 6);

        this.createField(600, 320, 'b.c.0+b.d.0', 7);

        this.createField(500, 440, 'c.0', 8);

        this.createField(700, 440, 'd.0', 9);

        this.createField(600, 560, '0', 10);


        // arrows
        this.add.image(150,260, 'arrow').rotation += 2.356;
        this.add.image(250,260, 'arrow').rotation += 0.785;
        this.add.image(100,380, 'arrow').setTint(0xFF0000).rotation += 1.57;
        this.add.image(300,380, 'arrow').setTint(0xFF0000).rotation += 1.57;
        this.add.image(150,500, 'arrow').setTint(0xFFAA00).rotation += 0.785;
        this.add.image(250,500, 'arrow').setTint(0x0000ff).rotation += 2.356;
        this.add.image(600,260, 'arrow').rotation += 1.57;
        this.add.image(550,380, 'arrow').setTint(0xFF0000).rotation += 2.3;
        this.add.image(650,380, 'arrow').setTint(0xFF0000).rotation += 0.785;
        this.add.image(550,500, 'arrow').setTint(0xFFAA00).rotation += 0.785;
        this.add.image(650,500, 'arrow').setTint(0x0000ff).rotation += 2.356;
     
        //initialize the adjacency matrix
        this.connected = this.createConnections(13);
        this.connected[0][1] = 1;
        this.connected[0][2] = 1;
        this.connected[1][3] = 1;
        this.connected[2][4] = 1;
        this.connected[3][5] = 1;
        this.connected[4][5] = 1;
        this.connected[6][7] = 1;
        this.connected[7][8] = 1;
        this.connected[7][9] = 1;
        this.connected[8][10] = 1;
        this.connected[9][10] = 1;

        // Initialize the observations matrix
        this.observations = this.createObservations(13);
        this.observations[0][1] = 'a';
        this.observations[0][2] = 'a';
        this.observations[1][3] = 'b';
        this.observations[2][4] = 'b';
        this.observations[3][5] = 'c';
        this.observations[4][5] = 'd';
        this.observations[6][7] = 'a';
        this.observations[7][8] = 'b';
        this.observations[7][9] = 'b';
        this.observations[8][10] = 'c';
        this.observations[9][10] = 'd';

        //create hero
        this.hero = new Hero(200, 200, field_0, this);

        // Create first defender
        this.defender = new Defender(this.enemy, this);
        this.defender.addEnemy(600, 200, field_defender);
    }
}
